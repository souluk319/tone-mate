...
발견한 문제와 재현 결과는 즉시 사용자에게 보고한다. 아래 절차의 이슈 발행·댓글 게시는 사용자 지시 후에만 수행하며, 수정은 해당 결함에 필요한 최소 범위로 제한한다.
...
ToneMate remote는 `https://github.com/souluk319/tone-mate.git`이며 첫 개발 작업장은 `feat/1-bootstrap-toolchain`이다.
